...
x = keras.layers.concatenate(model.outputs, name = 'joint_output')
this_model = Model(inputs=model.inputs, outputs= x)
this_model.compile(optimizer=keras.optimizers.Adam(lr=learning_rate), loss = util.naive_loss, metrics = [util.joint_acc_metric])
...
for dir_head in dir_heads:
      ...
      train_temp = train_temp.map(util.joint_parse_function_generator(dir_head))
      train_temp = train_temp.batch(8)  # Batch size to use
      test_temp = test.repeat()
      test_temp = test_temp.map(util.joint_parse_function_generator(dir_head))
      test_temp = test_temp.batch(8)
      ein = this_model.evaluate(train_temp.make_one_shot_iterator(),steps=1079)
      eout = this_model.evaluate(test_temp.make_one_shot_iterator(),steps=364)
      eins.append(ein)
      eouts.append(eout)
